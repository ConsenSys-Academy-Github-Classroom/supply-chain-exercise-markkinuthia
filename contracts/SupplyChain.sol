...
contract SupplyChain {
  ...
  // Create a modifer, `isOwner` that checks if the 
  ...
  modifier checkValue(uint _sku) {
    ...
    // (why it is before, _ checks for logic
    // before func)
    _;
  ...
  // an Item has been added?
   ...
   modifier forSale(uint _sku) {
      require((items[_sku].seller != address(0)) && (items[_sku].state == State.ForSale), "Items is not for Sale");
      _;
   }
